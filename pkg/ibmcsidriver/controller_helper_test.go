...
func TestIsValidCapacityIOPS4CustomClass(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			expectedError:  fmt.Errorf("invalid PVC size for custom class: <%v>. Should be in range [%d - %d]GiB", 18000, utils.MinimumVolumeDiskSizeInGb, utils.MaximumVolumeDiskSizeInGb),
		},
		{
			...
			requestIops:    5,
			expectedStatus: false,
			expectedError:  fmt.Errorf("invalid IOPS: <%v> for capacity: <%vGiB>. Should be in range [%d - %d]", 5, 6000, customCapacityIopsRanges[7].minIops, customCapacityIopsRanges[7].maxIops),
		},
	}
